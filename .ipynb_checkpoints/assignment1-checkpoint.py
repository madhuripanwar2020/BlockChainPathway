...
import pandas as pd
import hashlib
# Used AgGrid to display the DataFrame
...
# formalized data structure that consists of the `sender`, `receiver`, and `amount` attributes. 
...
@dataclass
class Record:
    # Define an attribute named `sender` of type `str`
    sender: str
    # Define an attribute named `receiver` of type `str`
    receiver: str
    # Define an attribute named `amount` of type `float`
    amount: float
...
sender_var =  str(st.text_area("sender"))

# @TODO:
# Add an input area where you can get a value for `receiver` from the user.
receiver_var =  str(st.text_area("receiver"))

# @TODO:
# Add an input area where you can get a value for `amount` from the user.
amount_var = st.number_input(label="amount",step=100.,format="%.2f")
...
if st.button("Add Block"):
    ...
    # and `amount` values
    new_block = Block(
        record = Record (sender_var, receiver_var, amount_var),
...
#Note - I am having some issues with loading this data in DataFrame & hence I am writing on screen w/o dataframe
pychain_df = pd.DataFrame(pychain.chain)
#st.write(pychain_df.)
AgGrid(pychain_df, height=200, fit_columns_on_grid_load=True)
